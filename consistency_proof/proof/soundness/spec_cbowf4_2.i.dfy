...
module CboWf4_2Proof {
    ...
    ensures Safety(c, v')
  {
    ...
    forall e: Event | e in v'.execution.pre.events
      ...
      ensures !((step.e, e) in v'.execution.ef())
    {

    }
    forall x | x in v'.execution.ef()
      ensures x in v.execution.ef()
    {
      var me := x.0;
      var es := x.1;
      assert me != step.e;
      assert ((me, es) in v.execution.wit.cbo - Library.compose(v.execution.wit.cbo, Library.compose(Library.iden(v.execution.CallbackTimeEvents()), v.execution.wit.cbo)));
    }
    forall x | x in v.execution.ef()
      ensures x in v'.execution.ef()
    {
      ...
      forall e: Event | e in v'.execution.CallbackTimeEvents()
        ensures !((me, e) in v'.execution.wit.cbo && (e, es) in v'.execution.wit.cbo)
      {}
      assert ((me, es) in v'.execution.wit.cbo - Library.compose(v'.execution.wit.cbo, Library.compose(Library.iden(v'.execution.CallbackTimeEvents()), v'.execution.wit.cbo)));
    }
  }
    ...
    ensures Safety(c, v')
  {
    ...
    assert (me, step.e) in v'.execution.ef() by {
    ...
    forall x | x in v'.execution.ef() - {(me, step.e)}
      ensures x in v.execution.ef()
    {
      var me' := x.0;
      var es := x.1;
    ...
    // assert v'.execution.ef() - {(me, step.e)} <= v.execution.ef();
    forall x | x in v.execution.ef()
      ensures x in v'.execution.ef()
    {
      var me := x.0;
    ...
    // assert v'.execution.ef() >= v.execution.ef();
  }
