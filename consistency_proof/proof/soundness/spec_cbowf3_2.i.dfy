...
module CboWf3_2Proof {
  ...
  ghost predicate Alt(c: Constants, v: Variables) {
    forall ee: Event, es: Event | WFEvictEnd(c, v, ee) && WFEvictStart(c, v, es) && (ee, es) in v.execution.wit.cbo ::
      exists ms: Event, me: Event :: (
        && WFMissStart(c, v, ms)
        && WFMissEnd(c, v, me)
        && (ms, me) in v.execution.pre.thd()
  ...
  lemma StartOnEvictCBPreservesSafety(c: Constants, v: Variables, v': Variables, step: Step)
    ...
    ensures Safety(c, v')
  {
    forall ee: Event | WFEvictEnd(c, v', ee)  && (ee, step.e) in v'.execution.wit.cbo
    ensures
      exists ms: Event, me: Event :: (
        && WFMissStart(c, v', ms)
        && WFMissEnd(c, v', me)
        && (ms, me) in v'.execution.pre.thd()
        && (ee, ms) in v'.execution.wit.cbo
        && (me, step.e) in v'.execution.wit.cbo
      )
    {
      assert ee.info.id.count < v.epochs[ee.addr].epoch;
    }
  }
  ...
  lemma AltImpliesCboWf3_2(c: Constants, v: Variables)
    requires Alt(c, v)
    requires AllEventsWellFormed(c, v)
    ensures v.execution.CboWf3_2()
  {
    forall x : (Event, Event) | (x in Library.compose(Library.iden(v.execution.EvictEnds()), Library.compose(v.execution.wit.cbo, Library.iden(v.execution.EvictStarts()))))
      ensures x in
      Library.compose(
        Library.iden(v.execution.EvictEnds()),
        Library.compose(
          v.execution.wit.cbo,
          Library.compose(
            Library.intersection(
              v.execution.pre.thd(),
              Library.prod(
                (v.execution.MissStarts()),
                (v.execution.MissEnds())
              )
            ),
            Library.compose(
              v.execution.wit.cbo,
