...
module CboWf3_1Proof {
  ...
  ghost predicate Alt(c: Constants, v: Variables) {
    forall me: Event, ms: Event | WFMissEnd(c, v, me) && WFMissStart(c, v, ms) && (me, ms) in v.execution.wit.cbo ::
      exists es: Event, ee: Event :: (
        && WFEvictStart(c, v, es)
        && WFEvictEnd(c, v, ee)
        && (es, ee) in v.execution.pre.thd()
        && (me, es) in v.execution.wit.cbo
        && (ee, ms) in v.execution.wit.cbo
      )
  }
  ...
  lemma BranchPreservesInv2(c: Constants, v: Variables, v': Variables, step: Step)
    ...
    ensures Inv2(c, v')
  {
    // weird instability issue. works in 3_2, 3_3, and 3_4 without issue
    assert EvictEpochExistingEventsN(c, v');
    assert LowerEpochExistingEventsN(c, v');
    assert InEpochExistingEventsN(c, v');
    assert MissEpochExistingEventsN(c, v');
  ...
  lemma StartOnMissCBPreservesSafety(c: Constants, v: Variables, v': Variables, step: Step)
    ...
    ensures Safety(c, v')
  {
    forall me: Event | WFMissEnd(c, v', me) && (me, step.e) in v'.execution.wit.cbo
      ensures exists es: Event, ee: Event :: (
        && WFEvictStart(c, v', es)
        && WFEvictEnd(c, v', ee)
        && (es, ee) in v'.execution.pre.thd()
        && (me, es) in v'.execution.wit.cbo
  ...
  lemma AltImpliesCboWf3_1(c: Constants, v: Variables)
    requires Alt(c, v)
    requires AllEventsWellFormed(c, v)
    ensures v.execution.CboWf3_1()
  {


    forall x : (Event, Event) | (x in Library.compose(Library.iden(v.execution.MissEnds()), Library.compose(v.execution.wit.cbo, Library.iden(v.execution.MissStarts()))))
      ensures x in
      Library.compose(
        Library.iden(v.execution.MissEnds()),
        Library.compose(
          v.execution.wit.cbo,
          Library.compose(
            ...
            Library.compose(
              v.execution.wit.cbo,
              Library.iden(v.execution.MissStarts())
            )
          )
        )
      )
