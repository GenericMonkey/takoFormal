...
module CboWf3_3Proof {
  ...
  ghost predicate Alt(c: Constants, v: Variables) {
    forall ms1: Event, ms2: Event | WFMissStart(c, v, ms1) && WFMissStart(c, v, ms2) && (ms1, ms2) in v.execution.wit.cbo ::
      exists me: Event :: (
        && WFMissEnd(c, v, me)
        && (ms1, me) in v.execution.pre.thd()
        && (me, ms2) in v.execution.wit.cbo
      )
  }
  ...
  lemma StartOnMissCBPreservesSafety(c: Constants, v: Variables, v': Variables, step: Step)
    ...
    ensures Safety(c, v')
  {
    forall ms1: Event | WFMissStart(c, v', ms1) && WFMissStart(c, v', step.e) && (ms1, step.e) in v'.execution.wit.cbo
      ensures exists me: Event :: (
        && WFMissEnd(c, v', me)
        && (ms1, me) in v'.execution.pre.thd()
        && (me, step.e) in v'.execution.wit.cbo
      )
    {
      assert ms1.info.id.count < v.epochs[ms1.addr].epoch;
    }
  }
  ...
  lemma AltImpliesCboWf3_3(c: Constants, v: Variables)
    requires Alt(c, v)
    requires AllEventsWellFormed(c, v)
    ensures v.execution.CboWf3_3()
  {
      ...
      )
    {
      var ms1 := x.0;
      var ms2 := x.1;
      assert WFMissStart(c, v, ms1);
      assert WFMissStart(c, v, ms2);
      var me :| (
        && WFMissEnd(c, v, me)
        && (ms1, me) in v.execution.pre.thd()
        && (me, ms2) in v.execution.wit.cbo
