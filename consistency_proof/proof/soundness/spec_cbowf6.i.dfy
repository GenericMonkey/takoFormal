...
module CboWf6Proof {
    ...
    ensures Safety(c, v')
  {
    ...
    assert v'.execution.CallbackTimeEvents() == v.execution.CallbackTimeEvents();
    forall x | x in v'.execution.viscb() && x.1.Es?
    ...
    ensures Safety(c, v')
  {
    ...
    if step.e.dirty {
      var wcb := v.epochs[step.e.addr].wcb.val;
      // assert wcb.Wcb?;
      assert (wcb, step.e) in v'.execution.viscb() by {
        assert (wcb, step.e) in v'.execution.wit.cbo -
          Library.compose(
            v'.execution.wit.cbo,
            Library.compose(
              Library.iden(v'.execution.WriteCBs() + v'.execution.CallbackTimeEvents()),
      ...
      ensures x in v'.execution.viscb()
    {
      ...
      var es := x.1;
      forall e: Event | e in (v'.execution.WriteCBs() + v'.execution.CallbackTimeEvents())
        ensures !((v_e, e) in v'.execution.wit.cbo && (e, es) in v'.execution.wit.cbo)
      {}
      assert (v_e, es) in v'.execution.wit.cbo -
        Library.compose(
          v'.execution.wit.cbo,
          Library.compose(
            Library.iden(v'.execution.WriteCBs() + v'.execution.CallbackTimeEvents()),
    ...
    // forall es: Event | es in v'.execution.pre.events && es.Es? && es.dirty
    //   ensures (exists wcb : Event :: wcb.Wcb? && (wcb, es) in v'.execution.viscb())
    // {
    //   if es == step.e {
    //     assert (exists wcb : Event :: wcb.Wcb? && (wcb, es) in v'.execution.viscb());
    //   } else {
    //     assert es in v.execution.pre.events;
    //     assert (exists wcb : Event :: wcb.Wcb? && (wcb, es) in v'.execution.viscb());
    //   }
    // }
    ...
    forall es: Event | es in v'.execution.pre.events && es.Es? && !es.dirty
      ensures (forall wcb : Event | wcb in v'.execution.pre.events && wcb.CBWrite() :: !((wcb, es) in v'.execution.viscb()))
    {
      if es != step.e {
        assert es in v.execution.pre.events;
        forall wcb : Event | wcb in v'.execution.pre.events && wcb.CBWrite()
          ensures !((wcb, es) in v'.execution.viscb())
        {
          assert wcb in v.execution.pre.events;
          assert !((wcb, es) in v.execution.viscb());
          if (wcb, es) in v.execution.wit.cbo {
            assert !((wcb, es) in v.execution.wit.cbo -
              Library.compose(
                v.execution.wit.cbo,
                Library.compose(
                  Library.iden(v.execution.WriteCBs() + v.execution.CallbackTimeEvents()),
            ...
            var mid :| mid in v.execution.WriteCBs() + v.execution.CallbackTimeEvents()
              && (wcb, mid) in v.execution.wit.cbo
              && (mid, es) in v.execution.wit.cbo;
            assert !((wcb, es) in v'.execution.viscb());
          }
        }
      }
